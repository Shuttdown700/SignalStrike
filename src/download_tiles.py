...
def download_map_tiles(center_coord,x_tolerance_m,y_tolerance_m,max_zoom=18,min_zoom=0):
    ...
    coord_box = get_coord_box(center_coord,x_tolerance_m,y_tolerance_m)
    parallel_threads = 4
    try:
        cmd = f'python3 "{get_tiles_file}" "{tile_url}" "{tile_dir}" --extent {coord_box.replace(","," ")} --minzoom {min_zoom} --maxzoom {max_zoom} --parallel {parallel_threads}'
        subprocess.run([cmd])
